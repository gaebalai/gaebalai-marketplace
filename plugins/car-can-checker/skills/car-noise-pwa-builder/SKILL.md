...
│   └── manifest.webmanifest # 아이콘은 사용자가 별도 추가 권장
├── pi/
│   ├── pi_server.py        # aiohttp HTTPS + WebSocket + python-can 디코더
│   ├── requirements.txt
│   └── certs/
│       ├── README.md       # mkcert 가이드 (templates에 미리 포함)
│       └── (cert.pem / key.pem — mkcert 발급 후 사용자가 배치)
└── README.md
```

> v0.1은 PWA 아이콘 자산을 동봉하지 않습니다. 홈 화면 아이콘이 필요하면 192×192 PNG를 `pwa/icon-192.png`로 저장하고 `manifest.webmanifest`에 `icons` 항목을 추가하세요.
...
라즈베리파이:
```bash
cd pi && python pi_server.py
# 환경변수로 호스트/포트/Origin 변경 가능:
#   HOST=0.0.0.0 PORT=8443 ALLOWED_ORIGIN=https://my-pi:8443 python pi_server.py
```
