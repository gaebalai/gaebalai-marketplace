...
### STEP 4. 이상음 후보 탐지 휴리스틱

| 패턴 | 의미 | 판정 |
|---|---|---|
| RPM↑ → 피크주파수 비례 ↑ | 엔진 회전성(차수성) 노이즈 — **정상** 가능성 높음 | 정상 분리 |
| 속도↑ → 피크주파수 비례 ↑ | 노면/타이어성 — **타이어/노면** | 분리 |
| 특정 RPM 대역에서만 피크(±50rpm) | **공진 또는 부품 결함** | ⚠ 후보 |
| RMS 갑자기 ↑, RPM 무관 | **충격/접촉음** | ⚠ 후보 |
| 조향각 변화 시 발생 | **서스펜션/조향 계통** | ⚠ 후보 |

### STEP 5. 리포트 출력

```
report_<ts>/
├── report.md          # 사람이 읽는 요약 + 후보 구간 표
├── overview.png       # 4단 패널 (스펙트로그램/RPM/속도/기어)
├── candidate_<n>.png  # 의심 구간 확대 + 메타데이터
└── correlations.csv   # RPM-주파수, 속도-주파수 상관 표
```
